...
const HomeScreen = ({navigation}) => {
	...
	const categoryItems = [
			{name: 'Furniture', iconName: 'table-furniture', category: 'service'},
			{name: 'Casual Bags', iconName: 'bag-personal-outline', category: 'accessory'},
			{name: 'Watches', iconName: 'watch', category: 'book'},
			{name: 'Shoes', iconName: 'shoe-formal', category: 'clothes'},
			
			
	];
	const filterProduct = index => {
		const currentProduct = products.filter((item)=>item?.category?.toLowerCase() == categoryItems[index].category,)[0]
		
		setFilteredCategory(currentProduct)
	};
		...
		<ScrollView showsVerticalScrollIndicator={false}>
			...
			<View style={{
				...
				<FlatList
					...
					data={Object.values(filteredCategory)}
